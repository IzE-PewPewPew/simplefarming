...
RegisterServerEvent('qb-simplefarming:cowmilking', function()
    ...
    TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['emptycowbucket'], "remove")
    Wait(1000)
...
QBCore.Functions.CreateCallback('qb-simplefarming:hasbucket', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then
        if Player.Functions.GetItemByName("emptycowbucket") ~= nil then
            cb(true)
...
QBCore.Functions.CreateCallback('qb-simplefarming:weapon_knife', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then
        if Player.Functions.GetItemByName("slaughterknife") ~= nil then
            cb(true)
        else
            cb(false)
        end
...
RegisterServerEvent('qb-simplefarming:getcowbucket', function()
    ...
    local source = source
    local Player = QBCore.Functions.GetPlayer(tonumber(source))
    --local emptyBucketCount = QBCore.Functions.GetItemByName("emptycowbucket").count

    -- if Player.Functions.HasItem('emptycowbucket') then
...
RegisterServerEvent('qb-simplefarming:cornpicking', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(tonumber(source))
    local cornfarming = math.random(1,2)
...
RegisterServerEvent('qb-simplefarming:grapepicking', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(tonumber(source))
    local grapepicking = math.random(1,3)
